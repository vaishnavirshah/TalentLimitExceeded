...
class _AddOrderState extends State<AddOrder> {
  ...
  final addressIp=TextEditingController();
  bool isVege = false;
  bool isNVeg = false;
  var description;
  var range;
 ...
 String _date = "Set Date";
  ...
  void submit()async{
    ...
    if(_time1==null || _day==null )
    {
      showDialog(
        ...
        builder: (ctx) {
          ...
          return AlertDialog(title:Text("Oops something went wrong"),content: FittedBox(
                child: Column(
              children: <Widget>[
                Text("Pick a valid date and time!"),IconButton(
      ...
      DateTime day6=_day;
      DateTime day4=  _day.add(Duration(hours:_time1.hour,minutes:_time1.minute,seconds:_time1.second ));
    ...
    var userData=await Firestore.instance.collection('donors').document(user.uid).get();
   try{ 
     setState(() {
          isLoading = true;
        });
    DocumentReference document11= Firestore.instance.collection('donors').document(user.uid).collection('orders').document();
    document11.setData(
      {
         'time':Timestamp.now(),
         'range':range,
   ...
   DocumentReference documentreference= Firestore.instance.collection('orders').document(document11.documentID);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isLoading ? Center(child: Container(child: CircularProgressIndicator(),width: 20,)) : new GestureDetector(
         onTap: () {
          ...
          child: Form(
               ...
               // SizedBox(height: 5),
                  ...
                  Container(
                    ...
                    child: TextFormField(
                     ...
                     validator:(value)
                     {
                       if(value.isEmpty || double.parse(value.trim())<7000000000||double.parse(value)<=0)
                       {
                  ...
                  FocusScope.of(context).unfocus();
                    DatePicker.showDatePicker(context,
                       ...
                       // minTime: DateTime.now(),
                        maxTime: DateTime.now().add(Duration(hours:24)), onConfirm: (date) {
                      print('confirm $date');
                      _date = '${date.day} - ${date.month} - ${date.year}';
                      setState(() { 
                        DateTime day5=date;
                        if(date.subtract(Duration(hours:24)).isAfter(DateTime.now()))
                        {
                           showDialog(
              context: context,
              builder: (ctx) {
            ...
            content: FittedBox(
              ...
              children: <Widget>[
                ...
                SizedBox(height: 10),
                    Text(' Pickup up time should be within 24 hours from now ',
              ...
              SizedBox(height:MediaQuery.of(context).size.height*0.005,),
               Container(
                ...
                Divider(),
                Container(child: Text('Food Category: ',style: TextStyle(fontSize:20,fontWeight: FontWeight.bold),),margin:EdgeInsets.all(10),),
                CheckboxListTile(
                  title: Text('Veg: ',style: TextStyle(fontSize:18,fontWeight: FontWeight.w500),),
                  value: isVege,
                ...
                CheckboxListTile(
                  title: Text(
                  'Non-Veg: ',style: TextStyle(fontSize:18,fontWeight: FontWeight.w500)
                  ),
                  value: isNVeg,
