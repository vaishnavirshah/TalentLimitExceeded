    ...
    if (isValid) {
      _formKey.currentState.save();
       try{
        ...
        authResult=await _auth.signInWithEmailAndPassword(email:_userEmail, password: _userPasswrod);
        var doc1= Firestore.instance.collection('users').document(authResult.user.uid);
      ...
      var message = 'An error occurred, pelase check your credentials!';
       if (err.message != null) {
        ...
        builder: (ctx) {
          ...
          return AlertDialog(title:Text("Oops something went wrong"),content: Container(
                height: MediaQuery.of(context).size.height*0.129,
                child: Column(
              children: <Widget>[
                Text(err.message==null?"sorry for incovinience":message,style:TextStyle(fontSize: 15),),IconButton(
                    icon: Icon(Icons.arrow_back),
                    onPressed: () {
                      Navigator.of(context).pop();
                    })])));});
      ...
      showDialog(
        ...
        builder: (ctx) {
          ...
          return AlertDialog(title:Text("Oops something went wrong"),content: Container(
                ...
                child: Column(
              children: <Widget>[
                Text("sorry for incovinience",style:TextStyle(fontSize: 15),),IconButton(
                    icon: Icon(Icons.arrow_back),
                    onPressed: () {
                      Navigator.of(context).pop();
                    })])));});
