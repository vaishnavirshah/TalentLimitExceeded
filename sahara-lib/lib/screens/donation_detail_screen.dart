...
class _DonationDetailScreenState extends State<DonationDetailScreen> {
    ...
    return showDialog(
        ...
        builder: (ctx) => AlertDialog(
              ...
              actions: <Widget>[
                FlatButton(
                    ...
                    var user11=await Firestore.instance.collection('receiver').document(user.uid).get();
                   String username1=user11['username'];
                    await Firestore.instance.collection('receiver').document(user.uid).collection('past orders').document().setData(
                      {
                           ...
                           'range': userData['range'],
                            'foodDescription': userData['description'],
        ...
        builder: (ctx) {
          ...
          return AlertDialog(title:Text("Oops something went wrong"),content: FittedBox(
                child: Column(
              children: <Widget>[
                Text("sorry for incovinience"),IconButton(
                    icon: Icon(Icons.arrow_back),
                    onPressed: () {
                      Navigator.of(context).pop();
                    })])));});
  ...
  Widget build(BuildContext context) {
    ...
    final rangeNumber = userData['range'].toString();
     DateTime time1=userData['time1'].toDate();
       ...
       //padding: EdgeInsets.only(top: 3.0, right: 4.0),
         child: ListTile(
                   title: Text('Time until the order expires:',
                      style:
                          TextStyle(fontWeight: FontWeight.bold, fontSize: 20)),subtitle: Align(alignment: Alignment.bottomLeft,
